...
batch_size = 16
epochs = 50
checkpoint_name="vgg19_1_CE_L1_MF"
...
def generator(data_x, data_y1, data_y2, data_y3, batch_size=32):
    ...
    while 1:
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            y3_train = data_y3[shuffled_batch]
            X_train = preprocess_img(X_train)
            y_train = {"classification": y1_train, "regression": y2_train, "gender": y3_train}
            yield (X_train, y_train)

train_generator = generator(train_images, one_hot_train_labels, train_labels, one_hot_train_gender_labels, batch_size=batch_size)
valid_generator = generator(valid_images, one_hot_valid_labels, valid_labels, one_hot_valid_gender_labels, batch_size=batch_size)
...
x3 = Dense(2, activation=None)(x3)
predictions_MF = Activation(tf.nn.softmax, name="gender")(x3)
...
# hyper-parameter
lossWeights = {"classification": 1.0, "regression": 0.05, "gender":0.5}

# Creating the final model 
model_final = Model(input = model.input, output = [predictions_CE, predictions_L1, predictions_MF])
